...
<template>
  <div class="dashboard-page">
    ...
    <div class="dashboard-header">
      <h1 class="header-title">📊 DASHBOARD SALES PARETO - {{ dashboardTitle }}</h1>
    </div>
      ...
      <div class="filters-container">
        <div class="filters-grid">
          ...
          <div class="period-group">
            ...
            <el-select v-model="filters.selectedYear" class="select-sm">
              <el-option
            ...
            <el-select v-model="filters.selectedWeek" class="select-sm">
              <el-option
          ...
          <div class="filters-group">
            ...
            <el-select
              v-model="filters.selectedFamily"
              collapse-tags
              class="select-full"
            ...
            <el-select
              v-model="filters.selectedTobacco"
              collapse-tags
              class="select-full"
          ...
          >
            <template #default="scope">
              <transition name="cell-update" mode="out-in">
                <div
                  :key="`${scope.row.Area}-${col.prop}-${sortState.prop}-${sortState.order}`"
                  class="cell-value"
                  :class="{ 'is-total': isTotalRow(scope.row) }"
...
import { Calendar, Filter, Histogram, Download, Loading, InfoFilled, Camera } from '@element-plus/icons-vue'
...
// Mapping konfigurasi berdasarkan route
...
const dashboardType = computed(() => {
  ...
  return DASHBOARD_CONFIG[type] || DASHBOARD_CONFIG.area
})

const dashboardTitle = computed(() => dashboardType.value.title)
const columnLabel = computed(() => dashboardType.value.column)
const groupingField = computed(() => dashboardType.value.field)
...
const filteredBrandsList = computed(() => {
  if (!fullDataCache.value || fullDataCache.value.length === 0) {
    return brandsList.value
  }

  let data = fullDataCache.value

  if (filters.selectedFamily.length > 0) {
    data = data.filter(r => filters.selectedFamily.includes(r.family))
  }

  if (filters.selectedTobacco.length > 0) {
    data = data.filter(r => filters.selectedTobacco.includes(r.tobacco_group))
  }
...
const dynamicColumns = computed(() => {
  ...
  return [
    { prop: `avgVolume${prevYear}`, label: `Avg. Volume ${prevYear}`, width: 140 },
    { prop: `avgVolume${year}`, label: `Avg. Volume ${year}`, width: 140 },
    { prop: 'w1', label: `Week ${getWeekLabel(currentWeek, -3)}`, width: 120 },
    { prop: 'w2', label: `Week ${getWeekLabel(currentWeek, -2)}`, width: 120 },
    { prop: 'w3', label: `Week ${getWeekLabel(currentWeek, -1)}`, width: 120 },
    { prop: 'w4', label: `Week ${getWeekLabel(currentWeek, 0)}`, width: 120 },
    { prop: 'growthLW', label: 'Growth LW (%)', width: 150, isPercentage: true },
    { prop: 'ctbr', label: 'CTBR (%)', width: 130, isPercentage: true },
    { prop: 'achYTD', label: 'ACH YTD (%)', width: 150, isPercentage: true },
...
const loadData = async () => {
  ...
  try {
    const selectedParam = allCycles.value.find(
      p => p.cycle_year === filters.selectedYear && p.week === filters.selectedWeek
    )
    ...
    const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PARETO}?${params}`
    const response = await fetch(url)
    ...
    if (filters.selectedFamily.length > 0) {
      results = results.filter(r => filters.selectedFamily.includes(r.family))
    }
    if (filters.selectedTobacco.length > 0) {
      results = results.filter(r => filters.selectedTobacco.includes(r.tobacco_group))
    }
...
const processTableData = (data) => {
  ...
  data.forEach(item => {
    ...
    if (!grouped[groupKey]) {
      grouped[groupKey] = {
        ...
        qty_lastweek_2: 0,
        qty_lastweek_3: 0,
        avg_sales_this_year: 0,
        avg_sales_last_year: 0,
        sales_ytd_this_year: 0,
        sales_ytd_last_year: 0,
        ytd_ae_this_year: 0
      }
    }

    grouped[groupKey].qty_current_week += item.qty_current_week || 0
    grouped[groupKey].qty_lastweek_1 += item.qty_lastweek_1 || 0
    grouped[groupKey].qty_lastweek_2 += item.qty_lastweek_2 || 0
    grouped[groupKey].qty_lastweek_3 += item.qty_lastweek_3 || 0
    grouped[groupKey].avg_sales_this_year += item.avg_sales_this_year || 0
    grouped[groupKey].avg_sales_last_year += item.avg_sales_last_year || 0
    grouped[groupKey].sales_ytd_this_year += item.sales_ytd_this_year || 0
    grouped[groupKey].sales_ytd_last_year += item.sales_ytd_last_year || 0
    grouped[groupKey].ytd_ae_this_year += item.ytd_ae_this_year || 0

    totalQtyCurrent += item.qty_current_week || 0
  })

  const rows = Object.entries(grouped).map(([groupName, values]) => {
    const growthLW = values.qty_lastweek_1 !== 0
      ? ((values.qty_current_week - values.qty_lastweek_1) / values.qty_lastweek_1) * 100
      : 0

    const ctbr = totalQtyCurrent !== 0
      ? (values.qty_current_week / totalQtyCurrent) * 100
      : 0

    const achYTD = values.ytd_ae_this_year !== 0
      ? (values.sales_ytd_this_year / values.ytd_ae_this_year) * 100
      : 0

    const growthYTD = values.sales_ytd_last_year !== 0
      ? ((values.sales_ytd_this_year - values.sales_ytd_last_year) / values.sales_ytd_last_year) * 100
      : 0
    ...
    return {
      ...
      w1: values.qty_lastweek_3,
      w2: values.qty_lastweek_2,
      w3: values.qty_lastweek_1,
      w4: values.qty_current_week,
      growthLW,
      ctbr,
      achYTD,
      growthYTD,
  ...
  top10.forEach((row, index) => { row.No = index + 1 })

  const prevYear = parseInt(periodInfo.year) - 1

  const top10Total = {
    No: '',
    Area: 'Total Top 10 Pareto',
    [`avgVolume${prevYear}`]: top10.reduce((sum, row) => sum + row[`avgVolume${prevYear}`], 0),
    [`avgVolume${periodInfo.year}`]: top10.reduce((sum, row) => sum + row[`avgVolume${periodInfo.year}`], 0),
    w1: top10.reduce((sum, row) => sum + row.w1, 0),
    w2: top10.reduce((sum, row) => sum + row.w2, 0),
    w3: top10.reduce((sum, row) => sum + row.w3, 0),
    w4: top10.reduce((sum, row) => sum + row.w4, 0),
    growthLW: top10.reduce((sum, row) => sum + row.growthLW, 0) / top10.length,
    ctbr: top10.reduce((sum, row) => sum + row.ctbr, 0),
    achYTD: top10.reduce((sum, row) => sum + row.achYTD, 0) / top10.length,
    growthYTD: top10.reduce((sum, row) => sum + row.growthYTD, 0) / top10.length,
    isTotal: true
  }

  const nonPareto = rows.slice(10)
  const nonParetoTotal = {
    No: '',
    Area: 'Total Non Pareto',
    [`avgVolume${prevYear}`]: nonPareto.reduce((sum, row) => sum + row[`avgVolume${prevYear}`], 0),
    [`avgVolume${periodInfo.year}`]: nonPareto.reduce((sum, row) => sum + row[`avgVolume${periodInfo.year}`], 0),
    w1: nonPareto.reduce((sum, row) => sum + row.w1, 0),
    w2: nonPareto.reduce((sum, row) => sum + row.w2, 0),
    w3: nonPareto.reduce((sum, row) => sum + row.w3, 0),
    w4: nonPareto.reduce((sum, row) => sum + row.w4, 0),
    growthLW: nonPareto.length > 0 ? nonPareto.reduce((sum, row) => sum + row.growthLW, 0) / nonPareto.length : 0,
    ctbr: nonPareto.reduce((sum, row) => sum + row.ctbr, 0),
    achYTD: nonPareto.length > 0 ? nonPareto.reduce((sum, row) => sum + row.achYTD, 0) / nonPareto.length : 0,
    growthYTD: nonPareto.length > 0 ? nonPareto.reduce((sum, row) => sum + row.growthYTD, 0) / nonPareto.length : 0,
    isTotal: true
  }
  ...
  const totalNasional = {
    No: '',
    Area: 'Total Nasional',
    [`avgVolume${prevYear}`]: rows.reduce((sum, row) => sum + row[`avgVolume${prevYear}`], 0),
    [`avgVolume${periodInfo.year}`]: rows.reduce((sum, row) => sum + row[`avgVolume${periodInfo.year}`], 0),
    w1: rows.reduce((sum, row) => sum + row.w1, 0),
    w2: rows.reduce((sum, row) => sum + row.w2, 0),
    w3: rows.reduce((sum, row) => sum + row.w3, 0),
    w4: rows.reduce((sum, row) => sum + row.w4, 0),
    growthLW: rows.reduce((sum, row) => sum + row.growthLW, 0) / rows.length,
    ctbr: rows.reduce((sum, row) => sum + row.ctbr, 0),
    achYTD: rows.reduce((sum, row) => sum + row.achYTD, 0) / rows.length,
    growthYTD: rows.reduce((sum, row) => sum + row.growthYTD, 0) / rows.length,
    isTotal: true
  }

  tableData.value = [...top10, top10Total, nonParetoTotal, totalNasional]
}

const downloadExcel = () => {
  try {
    if (!allProcessedData.value || allProcessedData.value.length === 0) {
      ElMessage.warning('⚠️ Tidak ada data untuk diexport')
      return
    }
    ...
    const wLabel = (offset) => `Week ${getWeekLabel(week, offset)}`

    const headerInfo = [
      ['DASHBOARD SALES PARETO - ' + dashboardTitle.value.toUpperCase()],
      [`Period: ${periodInfo.fromDate} - ${periodInfo.toDate}`],
      [`Year: ${year} | Week: ${week}`],
      []
    ]
    ...
    const detailRows = allProcessedData.value.map(row => [
      row.No,
      row.Area,
      row[`avgVolume${prevYear}`],
      row[`avgVolume${year}`],
      row.w1,
      row.w2,
      row.w3,
    ...
    const totalRows = tableData.value.filter(r => r.isTotal).map(row => [
      '',
      row.Area,
      row[`avgVolume${prevYear}`],
      row[`avgVolume${year}`],
...
const downloadTableImage = async () => {
  try {
    ...
    const tableHTML = `
      <div style="background: white; padding: 20px; font-family: Arial, sans-serif;">
        <h2 style="text-align: center; color: #1f77b4; margin-bottom: 20px;">
          📊 SALES PARETO - ${dashboardTitle.value} - Year ${periodInfo.year}, Week ${filters.selectedWeek}
        </h2>
        <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
          <thead>
            <tr style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
              <th style="border: 1px solid #ddd; padding: 12px 8px; text-align: center;">No</th>
              <th style="border: 1px solid #ddd; padding: 12px 8px; text-align: left;">${columnLabel.value}</th>
              <th style="border: 1px solid #ddd; padding: 12px 8px; text-align: right;">Avg. Volume ${prevYear}</th>
              <th style="border: 1px solid #ddd; padding: 12px 8px; text-align: right;">Avg. Volume ${periodInfo.year}</th>
              ...
              <th style="border: 1px solid #ddd; padding: 12px 8px; text-align: right;">ACH YTD (%)</th>
          ...
          <tbody>
            ${displayTableData.value.map(row => {
              ...
              const getCellStyle = (prop, value) => {
                ...
                if (prop === 'achYTD' && !isNaN(num)) {
                  if (num >= 100) {
                    style += 'background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; font-weight: 600; border-radius: 6px;'
                  } else if (num >= 95) {
                    style += 'background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%); color: #1f2937; font-weight: 600; border-radius: 6px;'
                  } else if (num < 95) {
                    style += 'background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); color: white; font-weight: 600; border-radius: 6px;'
                  }
                }
              ...
              return `
                <tr style="${rowStyle}">
                  ...
                  <td style="${getCellStyle('achYTD', row.achYTD)}">${formatNumber(row.achYTD)}%</td>
    ...
    canvas.toBlob((blob) => {
      ...
      link.href = url
      link.download = `Sales_Pareto_${dashboardTitle.value}_${periodInfo.year}_Week${filters.selectedWeek}_${new Date().getTime()}.png`
...
.period-group {
  display: grid;
  grid-template-columns: auto minmax(90px, 110px) minmax(100px, 120px);
  gap: 0.5rem;
  align-items: center;
  white-space: nowrap;
}
...
.filters-group {
  display: grid;
  grid-template-columns: auto minmax(160px, 1fr) minmax(160px, 1fr) minmax(220px, 2fr);
  gap: 0.5rem;
  min-width: 0;
}
